...
class PagesController extends Controller
{
    ...
    public function history(){
        $user = User::find(Auth::id());
        $bufferPostings = BufferPosting::with('accountInfo', 'groupInfo')->where('user_id', $user->id)->paginate(25);
        return view('pages.history', compact('bufferPostings', 'user'));
    }
    ...
    public function historySearch(Request $request){
        ...
        $bufferPostings = DB::table('buffer_postings as bp')
            ->join('social_post_groups as spg', 'bp.group_id', '=', 'spg.id')
            ->join('social_accounts as sa', 'bp.account_id', '=', 'sa.id')
            ->where('bp.post_text','LIKE','%'.$text.'%')
            ->where('bp.user_id', $user->id)
            ->where('bp.created_at','LIKE', $date.'%')
            ->orWhere('spg.type', $group_type)
            ->select('bp.*', 'spg.name as group_name','spg.type as group_type', 'sa.type as account_type', 'sa.avatar as account_avatar', 'sa.name as account_name')
            ->paginate(25);
        return view('pages.history_search', compact('bufferPostings', 'user', 'text', 'date','group_type' ));
